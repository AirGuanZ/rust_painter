...
impl BxDF for MulBxDF {
    ...
    fn sample(&self, v: &Vec3f, n: u32) -> Vec<Vec3f> {
        (0..n).map(|_| self.sample_once(v)).collect()
    }
    ...
    fn sample_upper(&self, v: &Vec3f, n: u32) -> Vec<Vec3f> {
        (0..n).map(|_| self.sample_once_upper(v)).collect()
    }
...
impl MulBxDF {
    fn sample_once(&self, v: &Vec3f) -> Vec3f {
        ...
        match self.t {
            BxDFType::BRDF => df.sample_upper(v, 1)[0].clone(),
            BxDFType::BSDF => df.sample(v, 1)[0].clone(),
        }
    }

    fn sample_once_upper(&self, v: &Vec3f) -> Vec3f {
        if rand::random::<Real>() < self.afac {
            self.a.sample_upper(v, 1)[0].clone()
        } else {
            self.b.sample_upper(v, 1)[0].clone()
        }
    }
